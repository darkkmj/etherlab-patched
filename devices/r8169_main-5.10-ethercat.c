...
static int rtl8169_poll(struct napi_struct *napi, int budget)
{
	...
	if (!tp->ecdev && work_done < budget && napi_complete_done(napi, work_done))
		rtl_irq_enable(tp);
...
static void  ec_poll(struct net_device *dev)
{
	...
	if (status & LinkChg)
		ecdev_set_link(tp->ecdev,
			 RTL_R8(tp, PHYstatus) & LinkStatus ? 1 : 0);

	rtl8169_poll(&tp->napi, NUM_TX_DESC);
	return;
}
...
static int rtl_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (tp->ecdev) {
		...
		netdev_info(dev, "%s, %pM, XID %03x, IRQ %d, chipseet %i\n",
				rtl_chip_infos[chipset].name, dev->dev_addr, xid,
				pci_irq_vector(pdev, 0), chipset);
