...
static void dump_pkt_data(unsigned char *pData, unsigned int size)
{
	int i;
...
static void dump_queue_data(struct igb_adapter *adapter)
{
	...
	*/
	int n, i;
	for (n = 0; n < 1; n++) {
		struct igb_ring *rx_ring_temp = adapter->rx_ring[n];
		pr_info("------------------------------------\n");
		pr_info("RX QUEUE INDEX = %d\n", rx_ring_temp->queue_index);
		pr_info("------------------------------------\n");
		pr_info("R  [desc]      [ PktBuf     A0] [  HeadBuf   DD] [bi->dma       ] [bi->skb] <-- Adv Rx Read format\n");
		pr_info("RWB[desc]      [PcsmIpSHl PtRs] [vl er S cks ln] ---------------- [bi->skb] <-- Adv Rx Write-Back format\n");

		for (i = 0; i < rx_ring_temp->count; i++) {
			const char *next_desc;
			struct igb_rx_buffer *buffer_info = &rx_ring_temp->rx_buffer_info[i];
			union e1000_adv_rx_desc *rx_desc_temp = IGB_RX_DESC(rx_ring_temp, i);
			struct my_u0 { u64 a; u64 b; } *u0;
			u0 = (struct my_u0 *)rx_desc_temp;
			u32 staterr = le32_to_cpu(rx_desc_temp->wb.upper.status_error);

			if (i == rx_ring_temp->next_to_use)
				next_desc = " NTU";
			else if (i == rx_ring_temp->next_to_clean)
				next_desc = " NTC";
			else
				next_desc = "";
			...
			if (staterr & E1000_RXD_STAT_DD) {
				...
				if (netif_msg_pktdata(adapter) &&
					...
					print_hex_dump(KERN_INFO, "",
						...
						igb_rx_bufsz(rx_ring_temp), true);
				}
			}
		}
	}
...
#define IGB_GSO_PARTIAL_FEATURES (NETIF_F_GSO_GRE | \
	...
	/* set ecdev before igb_sw_init() because of MSI(-X) enable or disable */
...
static int igb_sw_init(struct igb_adapter *adapter)
{
	...
	if (igb_init_interrupt_scheme(adapter, bEnableMSIx)) {
...
static int igb_clean_rx_irq(struct igb_q_vector *q_vector, const int budget)
{
	...
	while (likely(total_packets < budget)) {
		...
		if (adapter->ecdev) {
			...
			adapter->ec_watchdog_jiffies = jiffies;
			igb_reuse_rx_page(rx_ring, rx_buffer);      // fix memory leak
		}
		else if (IS_ERR(skb)) {
